...
file_path = 'd:/goat/SI-AKREDITASI/frontend/src/app/tabel/sarpras-pendidikan/page.js'
            ...
            <Card variant="default" className="relative overflow-hidden group !p-5 border-transparent shadow-sm hover:shadow-md transition-all duration-300">
              ...
              <div className="relative z-10 flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="text-[0.7rem] font-bold text-slate-400 uppercase tracking-widest mb-1">Total Sarana</p>
            ...
            <div className="overflow-x-auto custom-scrollbar">
              <table className="w-full text-left whitespace-nowrap">
                <thead className="bg-slate-50/80 border-b border-slate-200">
                  <tr className="text-[0.7rem] font-bold text-slate-500 uppercase tracking-wider text-center">
                    <th className="px-4 py-5 border-r border-slate-200">No</th>
                    <th className="px-6 py-5 border-r border-slate-200 text-left">Nama Prasarana</th>
                    <th className="px-4 py-5 border-r border-slate-200">Daya Tampung</th>
                    <th className="px-4 py-5 border-r border-slate-200">Luas (m²)</th>
                    <th className="px-4 py-5 border-r border-slate-200">Milik (M) / Sewa (W)</th>
                    <th className="px-4 py-5 border-r border-slate-200">
                      <div className="flex flex-col items-center">
                        <span>Lisensi</span>
                      </div>
                    </th>
                    <th className="px-6 py-5 border-r border-slate-200 text-left">Perangkat</th>
                    <th className="px-6 py-5 border-r border-slate-200 text-left">Info Tambahan</th>
                    <th className="px-4 py-5 border-r border-slate-200">Bukti</th>
                    <th className="px-4 py-5">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {(showTrash ? trashData : activeData).map((item, idx) => (
                    <tr key={item.id_5_2} className="hover:bg-slate-50/80 transition-colors group bg-white">
                      <td className="px-4 py-6 border-r border-slate-200 text-center font-bold text-slate-500 text-[0.75rem]">{idx + 1}</td>
                      <td className="px-6 py-6 border-r border-slate-200 text-left whitespace-normal">
                      ...
                      <td className="px-4 py-6 border-r border-slate-200 text-center font-bold text-slate-600">{item.daya_tampung || 0}</td>
                      <td className="px-4 py-6 border-r border-slate-200 text-center font-bold text-slate-600">{item.luas_ruang || 0}</td>
                      <td className="px-4 py-6 border-r border-slate-200 text-center">
                        <span 
                          title={item.status_milik === 'M' ? 'Milik Sendiri' : 'Sewa'}
                          className={`px-3 py-1 rounded-lg text-[0.65rem] font-black uppercase tracking-widest border ${item.status_milik === 'M' ? 'bg-blue-50 border-blue-200 text-blue-600' : 'bg-slate-100 border-slate-200 text-slate-500'}`}
                        >
                      ...
                      <td className="px-4 py-6 border-r border-slate-200 text-center">
                        <span 
                          ...
                          className={`px-3 py-1 rounded-lg text-[0.65rem] font-black uppercase tracking-widest border ${item.status_lisensi === 'L' ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : item.status_lisensi === 'P' ? 'bg-violet-50 border-violet-200 text-violet-600' : 'bg-red-50 border-red-200 text-red-600'}`}
                        >
                          {item.status_lisensi}
                        </span>
                      </td>
                      <td className="px-6 py-6 border-r border-slate-200 text-left text-[0.75rem] text-slate-600 font-medium whitespace-normal max-w-[200px] leading-relaxed italic">{item.perangkat || '-'}</td>
                      <td className="px-6 py-6 border-r border-slate-200 text-left text-[0.75rem] text-slate-600 font-medium whitespace-normal">{item.info_tambahan || '-'}</td>
                      <td className="px-4 py-6 border-r border-slate-200 text-center">
                        {item.link_bukti ? (
                      ...
                      <td className="px-4 py-6 align-middle text-center">
                        {showTrash ? (
                           <div className="flex items-center justify-center gap-2">
                             <button onClick={() => handleRestore(item.id_5_2)} className="px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100 transition font-bold text-[0.65rem] uppercase tracking-widest border border-emerald-100 flex items-center gap-1"><RotateCcw size={12}/> Restore</button>
                             <button onClick={() => handleHardDelete(item.id_5_2)} className="px-3 py-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition font-bold text-[0.65rem] uppercase tracking-widest border border-red-100 flex items-center gap-1"><Trash size={12}/> Hapus</button>
                           </div>
                        ) : (
                          <div className="flex justify-center gap-2">
                            ...
                            <button onClick={() => handleSoftDelete(item.id_5_2)} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg transition-all duration-300 border border-transparent hover:border-red-100" title="Hapus"><Trash2 size={16} /></button>
                          </div>
                        )}
                      </td>
                    </tr>
