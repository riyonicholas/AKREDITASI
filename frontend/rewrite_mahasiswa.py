...
replacements = [
    ...
    (r'border-transparent border-2 focus:border-emerald-500 focus:bg-gray-900', r'border-slate-200 bg-slate-50 focus:border-emerald-500 focus:bg-white focus:shadow-[0_0_0_3px_rgba(16,185,129,0.2)] border'),
...
for file_path in files_to_rewrite:
    ...
    content = re.sub(r'<button onClick=\{handleAddMain\}.*?>\s*<Plus size=\{18\} />\s*<span>(.*?)</span>\s*</button>', r'<Button onClick={handleAddMain}>\n                <Plus size={18} />\n                <span>{showForm ? \'Tutup Form\' : \'\1\'}</span>\n              </Button>', content)
    ...
    content = re.sub(r'<button type="button" onClick=\{resetForm\} className="px-6 py-3 bg-slate-100 text-slate-500 rounded-xl hover:bg-slate-200 transition font-medium">\s*Batal\s*</button>', r'<Button type="button" variant="ghost" onClick={resetForm}>Batal</Button>', content)
    content = re.sub(r'<button type="submit" className="px-6 py-3 bg-violet-600 text-slate-800 rounded-xl hover:bg-violet-700 transition font-medium">\s*(\{editingId \? \'Update Data\' : \'.*?\'\})\s*</button>', r'<Button type="submit">\1</Button>', content)
