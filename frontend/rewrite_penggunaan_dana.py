...
file_path = 'd:/goat/SI-AKREDITASI/frontend/src/app/tabel/penggunaan-dana/page.js'
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card variant="default" className="relative overflow-hidden group !p-5 border-transparent shadow-sm hover:shadow-md transition-all duration-300">
              ...
              <div className="relative z-10 flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="text-[0.7rem] font-bold text-slate-400 uppercase tracking-widest mb-1">Total Dana (TS)</p>
            ...
            <Card variant="default" className="relative overflow-hidden group !p-5 border-transparent shadow-sm hover:shadow-md transition-all duration-300">
              ...
              <div className="relative z-10 flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="text-[0.7rem] font-bold text-slate-400 uppercase tracking-widest mb-1">Rata-rata</p>
                  <p className="text-2xl font-black text-slate-800 tracking-tight leading-none">{formatRupiah(pivotedDataInfo.data.length ? pivotedDataInfo.data.reduce((acc, curr) => acc + (curr.ts || 0), 0) / pivotedDataInfo.data.length : 0)}</p>
                </div>
              </div>
            </Card>
            ...
            <div className="overflow-x-auto custom-scrollbar">
              <table className="w-full text-left whitespace-nowrap">
                <thead className="bg-slate-50/80 border-b border-slate-200">
                  {showTrash ? (
                    <tr>
                      <th className="px-8 py-5 text-[0.7rem] font-bold text-slate-500 uppercase tracking-wider border-r border-slate-200">Penggunaan</th>
                    ...
                    <tr>
                      <th className="px-8 py-5 text-[0.7rem] font-bold text-slate-500 uppercase tracking-wider border-r border-slate-200">Penggunaan Dana</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {showTrash ? (
                    trashData.map((item) => (
                      <tr key={item.id_penggunaan} className="hover:bg-orange-50/30 transition-colors group bg-white">
                        <td className="px-8 py-6 border-r border-slate-200 last:border-0">
                          <div className="text-sm font-semibold text-slate-800">{item.nama_penggunaan}</div>
                        </td>
                        ...
                        <td className="px-8 py-6">
                          <div className="inline-flex items-center gap-2">
                            <button onClick={() => handleRestore(item.id_penggunaan)} className="px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100 transition font-bold text-[0.65rem] uppercase tracking-widest border border-emerald-100 flex items-center gap-1"><RotateCcw size={12}/> Restore</button>
                            <button onClick={() => handleHardDelete(item.id_penggunaan)} className="px-3 py-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition font-bold text-[0.65rem] uppercase tracking-widest border border-red-100 flex items-center gap-1"><Trash size={12}/> Hapus</button>
                          </div>
                        </td>
                      </tr>
                    ))
                    ...
                    pivotedDataInfo.data.map((row, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/80 transition-colors group bg-white">
                        <td className="px-8 py-6 border-r border-slate-200 last:border-0 whitespace-normal">
                          <div className="text-sm font-semibold text-slate-800 group-hover:text-violet-600 transition-colors leading-relaxed">{row.nama_penggunaan}</div>
                          {row.link_bukti && (
                            <a href={row.link_bukti} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 mt-2 text-blue-500 hover:text-blue-600 font-bold text-[0.65rem] uppercase tracking-wider bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100 transition-colors">
                              ...
                              if (currentYearItem) {
                                return (
                                  <>
                                    ...
                                    <button onClick={() => handleSoftDelete(currentYearItem.id_penggunaan)} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg transition" title="Hapus TS"><Trash2 size={16} /></button>
                                  </>
                                );
                              } else {
                                return (
