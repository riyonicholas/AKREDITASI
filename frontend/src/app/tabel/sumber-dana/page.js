...
export default function SumberDanaPage() {
  ...
  useEffect(() => {
    if (prodiList.length > 0 && (!filterProdi || filterProdi === '')) {
      const savedProdi = localStorage.getItem('sumberDana_filterProdi');
      if (savedProdi && prodiList.some(p => p.id_prodi.toString() === savedProdi)) {
        ...
        if (tiProdi) {
          ...
          localStorage.setItem('sumberDana_filterProdi', tiProdi.id_prodi.toString());
        } else if (prodiList.length > 0) {
          setFilterProdi(prodiList[0].id_prodi.toString());
          localStorage.setItem('sumberDana_filterProdi', prodiList[0].id_prodi.toString());
        }
      }
    }
  ...
  useEffect(() => {
    if (!showTrash && (!filterTahun || filterTahun === '')) {
      const savedTahun = localStorage.getItem('sumberDana_filterTahun');
      if (savedTahun && tahunList.some(t => t.id_tahun.toString() === savedTahun)) {
        ...
        if (targetTahun) {
          ...
          localStorage.setItem('sumberDana_filterTahun', targetTahun.id_tahun.toString());
        }
      }
    }
  ...
  const handleProdiChange = (e) => {
    ...
    setFilterProdi(val);
    localStorage.setItem('sumberDana_filterProdi', val);
  };

  const handleTahunChange = (e) => {
    const val = e.target.value;
    setFilterTahun(val);
    localStorage.setItem('sumberDana_filterTahun', val);
  };
  ...
  const handleRestore = async (id) => {
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`http://localhost:5000/api/keuangan/1a2-sumber-pendanaan/restore/${id}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const result = await res.json();
  ...
  const handleHardDelete = async (id) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`http://localhost:5000/api/keuangan/1a2-sumber-pendanaan/hard/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const result = await res.json();
          ...
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
            <Card variant="default" className="relative overflow-hidden group h-[44px] !p-0 px-4 flex items-center border-transparent shadow-sm hover:shadow-md transition-all duration-300">
              <div className="relative z-10 flex items-center gap-3 w-full">
                ...
                <div className="flex-1 min-w-0 flex items-center gap-2">
                  <p className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest shrink-0 hidden xl:block">Total Pendanaan</p>
            ...
            <Card variant="default" className="relative overflow-hidden group h-[44px] !p-0 px-4 flex items-center border-transparent shadow-sm hover:shadow-md transition-all duration-300">
              <div className="relative z-10 flex items-center gap-3 w-full">
                ...
                <div className="flex-1 min-w-0 flex items-center gap-2">
                  <p className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest shrink-0 hidden xl:block">Sumber Aktif</p>
                  <p className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest shrink-0 xl:hidden">Aktif</p>
                  <div className="h-3 w-px bg-slate-200 shrink-0"></div>
                  <p className="text-sm font-black text-slate-800 tracking-tight truncate">{pivotedDataInfo.data.length}</p>
                </div>
              </div>
            </Card>
          </div>
            ...
            <div className="overflow-x-auto custom-scrollbar">
              <table className="w-full text-left whitespace-nowrap">
                <thead className="bg-[#1E3A8A]">
                  {showTrash ? (
                    <tr>
                      <th className="px-8 py-5 text-xs font-bold text-slate-100 uppercase tracking-wider border-r border-white/20">Sumber Dana</th>
                      ...
                      <th className="px-8 py-5 text-xs font-bold text-slate-100 uppercase tracking-wider border-r border-white/20">Aksi</th>
                    </tr>
                  ) : (
                    <tr>
                      <th className="px-8 py-5 text-xs font-bold text-slate-100 uppercase tracking-wider border-r border-white/20">Sumber Pendanaan</th>
                      ...
                      <th className="px-8 py-5 text-xs font-bold text-slate-100 uppercase tracking-wider border-r border-white/20">Link Bukti</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {showTrash ? (
                    trashData.map((item) => (
                      <tr key={item.id_sumber} className="hover:bg-violet-50/40 even:bg-slate-50/40 transition-colors group border-b border-slate-200">
                        <td className="px-8 py-6 border-r border-slate-200 last:border-0">
                          <div className="text-sm text-slate-800">{item.nama_sumber}</div>
                        </td>
                        ...
                        <td className="px-8 py-6 border-r border-slate-200">
                          <div className="inline-flex items-center gap-2">
                            <button onClick={() => handleRestore(item.id_sumber)} className="p-2 bg-white border border-slate-200 text-emerald-600 hover:bg-emerald-50 hover:border-emerald-200 shadow-sm rounded-lg transition" title="Restore"> <RotateCcw size={17} /></button>
                            <button onClick={() => handleHardDelete(item.id_sumber)} className="p-2 bg-white border border-slate-200 text-red-600 hover:bg-red-50 hover:border-red-200 shadow-sm rounded-lg transition" title=" "> <Trash2 size={17} /></button>
                          </div>
                        </td>
                      </tr>
                    ))
                    ...
                    pivotedDataInfo.data.map((row, idx) => (
                      <tr key={idx} className="hover:bg-violet-50/40 even:bg-slate-50/40 transition-colors group border-b border-slate-200">
                        <td className="px-8 py-6 border-r border-slate-200 last:border-0 whitespace-normal">
                          <div className="text-sm text-slate-800 group-hover:text-violet-600 transition-colors leading-relaxed">{row.nama_sumber}</div>
                        </td>
                        ...
                        <td className="px-8 py-6 border-r border-slate-200 last:border-0">
                          {row.link_bukti && (
                            <a href={row.link_bukti} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-blue-500 hover:text-blue-600 text-[0.65rem] uppercase tracking-wider bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100 transition-colors">
                              ...
                              if (currentYearItem) {
                                return (
                                  <>
                                    ...
                                    <button onClick={() => handleSoftDelete(currentYearItem.id_sumber)} className="p-2 bg-white border border-slate-200 text-red-600 hover:bg-red-50 hover:border-red-200 shadow-sm rounded-lg transition" title="Hapus TS"><Trash2 size={17} /></button>
                                  </>
                                );
                              } else {
                                return (
