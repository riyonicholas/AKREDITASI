        ...
        {showForm && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200 custom-scrollbar rounded-2xl">
              <Card title={editingId ? 'Edit Data Sarpras' : 'Input Sarpras Baru'} icon={<Layout className="text-violet-500" size={20}/>} variant="default" className="!p-0 shadow-2xl border-0 overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="md:col-span-2">
                    ...
                    <div className="relative">
                      ...
                      >
                        <span className="truncate">{formData.id_prodi ? prodiList.find(p => String(p.id_prodi) === String(formData.id_prodi))?.nama_prodi : 'Pilih Program Studi'}</span>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <div 
                        ...
                        className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 px-4 text-[0.9rem] text-slate-800 cursor-pointer flex justify-between items-center transition-all hover:border-violet-300"
                      >
                        <span className="truncate">{formData.status_milik === 'M' ? 'Milik Sendiri (M)' : formData.status_milik === 'W' ? 'Sewa (W)' : 'Status Milik'}</span>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <div 
                        ...
                        className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 px-4 text-[0.9rem] text-slate-800 cursor-pointer flex justify-between items-center transition-all hover:border-violet-300"
                      >
                        <span className="truncate">{formData.status_lisensi === 'L' ? 'Berlisensi (L)' : formData.status_lisensi === 'P' ? 'Public Domain (P)' : formData.status_lisensi === 'T' ? 'Tidak Berlisensi (T)' : 'Status Lisensi'}</span>
