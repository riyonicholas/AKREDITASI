...
export default function ProfilLulusanPage() {
  ...
  const handleDelete = async (id) => {
    const isConfirmed = await showConfirm('Yakin ingin menghapus profil lulusan ini secara permanen?');
      ...
      <div className="w-full p-6 md:p-8">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight m-0">Master Profil Lulusan (PL)</h1>
            <p className="text-slate-500 mt-1.5 text-sm">Kelola profil lulusan Program Studi sebagai acuan capaian pembelajaran</p>
          </div>
        ...
        {showForm && (
          <div className="mb-8 animate-in slide-in-from-top-4 duration-500">
            <Card title={editingId ? 'Edit Data Profil Lulusan' : 'Input Data Profil Lulusan Baru'} icon={<Plus className="text-violet-500" size={20}/>} variant="default" className="!p-0">
              <form onSubmit={handleSubmit} className="p-6">
                <div className="grid grid-cols-1 gap-6">
                  ...
                  <div>
                    <label className="text-xs font-bold uppercase tracking-widest text-slate-600 mb-2 block">Deskripsi Profil Lulusan</label>
                    <div className="relative">
                      <textarea 
                ...
                <div className="flex justify-end gap-3 pt-6 mt-6 border-t border-slate-100">
                  ...
                  <Button type="submit">{editingId ? 'Simpan Perubahan' : 'Tambah Profil Lulusan'}</Button>
                </div>
              </form>
            </Card>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
          {loading ? (
            ...
            <div className="p-20 text-center flex flex-col items-center">
              ...
              <p className="text-slate-500 font-bold text-lg tracking-tight">Belum ada data profil lulusan untuk prodi ini</p>
            </div>
          ) : (
              ...
              <table className="w-full min-w-max text-left whitespace-nowrap">
                <thead className="bg-[#1E3A8A]">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-xs font-bold text-slate-100 uppercase tracking-widest w-44 text-center border-r border-white/20">Kode PL</th>
                    <th className="px-6 py-4 text-xs font-bold text-slate-100 uppercase tracking-widest border-r border-white/20">Deskripsi Profil Lulusan</th>
                ...
                <tbody className="divide-y divide-slate-200">
                  {data.map((item, index) => (
                    <tr key={item.id_pl} className="hover:bg-violet-50/40 even:bg-slate-50/40 transition-colors group border-b border-slate-200">
                      ...
                      <td className="px-6 py-4 border-r border-slate-200">
                        <div className="text-[0.95rem] text-slate-900 leading-relaxed whitespace-pre-wrap">{item.deskripsi_pl}</div>
                      </td>
                      <td className="px-6 py-4 text-center border-r border-slate-200">
                        <div className="flex justify-center gap-2">
                          ...
                          <button onClick={() => handleDelete(item.id_pl)} className="p-2 bg-white border border-slate-200 text-red-600 hover:bg-red-50 hover:border-red-200 shadow-sm rounded-lg transition" title="Hapus"><Trash2 size={18} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
