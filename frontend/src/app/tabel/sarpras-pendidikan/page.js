    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.data.length > 0 && !filterIdProdi) {
          const savedProdi = localStorage.getItem('sarprasPendidikan_filterProdi');
          if (savedProdi && result.data.some(p => p.id_prodi.toString() === savedProdi)) {
            ...
            localStorage.setItem('sarprasPendidikan_filterProdi', result.data[0].id_prodi.toString());
          }
        }
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-50 pt-2 pb-10">
      <div className="w-full p-6 md:p-8">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight m-0">Sarpras Pendidikan (5.2)</h1>
            <p className="text-slate-500 mt-1.5 text-sm">Kelola data sarana dan prasarana laboratorium / ruang</p>
            
            {/* Legend Section */}
            <div className="flex flex-wrap gap-4 mt-4">
              <div className="flex items-center gap-6 px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl shadow-sm">
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-black text-violet-600 bg-violet-50 px-1.5 py-0.5 rounded border border-violet-200">L</span>
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Berlisensi</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-black text-violet-600 bg-violet-50 px-1.5 py-0.5 rounded border border-violet-200">P</span>
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Public Domain</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-black text-violet-600 bg-violet-50 px-1.5 py-0.5 rounded border border-violet-200">T</span>
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Tidak Berlisensi</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <Card variant="default" className="relative overflow-hidden group h-[44px] !p-0 px-4 flex items-center border-transparent shadow-sm hover:shadow-md transition-all duration-300">
              <div className="relative z-10 flex items-center gap-3 w-full">
                ...
                <div className="flex-1 min-w-0 flex items-center gap-2">
                  <p className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest shrink-0 hidden xl:block">Total Sarana</p>
              ...
              <div className="relative">
                <select 
                  ...
                  onChange={(e) => {
                    ...
                    localStorage.setItem('sarprasPendidikan_filterProdi', e.target.value);
                  }} 
              ...
              <table className="w-full text-left whitespace-nowrap">
                <thead className="bg-[#1E3A8A]">
                  <tr className="text-xs font-bold text-slate-100 uppercase tracking-wider text-center">
                    <th className="px-4 py-5 border-r border-white/20 font-bold">No</th>
                    <th className="px-6 py-5 border-r border-white/20 text-left font-bold">Nama Prasarana</th>
                    <th className="px-4 py-5 border-r border-white/20 font-bold">Daya Tampung</th>
                    <th className="px-4 py-5 border-r border-white/20 font-bold">Luas (m²)</th>
                    <th className="px-4 py-5 border-r border-white/20 font-bold">Milik (M) / Sewa (W)</th>
                    <th className="px-4 py-5 border-r border-white/20 font-bold">
                      <div className="flex flex-col items-center">
                        <span>Lisensi</span>
                      </div>
                    </th>
                    <th className="px-6 py-5 border-r border-white/20 text-left font-bold">Perangkat</th>
                    <th className="px-6 py-5 border-r border-white/20 text-left font-bold">Info Tambahan</th>
                    <th className="px-4 py-5 border-r border-white/20 font-bold">Bukti</th>
                    <th className="px-4 py-5 font-bold border-r border-white/20">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {(showTrash ? trashData : activeData).map((item, idx) => (
                    <tr key={item.id_5_2} className="hover:bg-violet-50/40 even:bg-slate-50/40 transition-colors group border-b border-slate-200">
                      <td className="px-4 py-4 border-r border-slate-200 text-center text-slate-500 text-[0.75rem]">{idx + 1}</td>
                      <td className="px-6 py-4 border-r border-slate-200 text-left whitespace-normal">
                      ...
                      <td className="px-4 py-4 border-r border-slate-200 text-center text-slate-600">{item.daya_tampung || 0}</td>
                      <td className="px-4 py-4 border-r border-slate-200 text-center text-slate-600">{item.luas_ruang || 0}</td>
                      <td className="px-4 py-4 border-r border-slate-200 text-center">
                        <span 
                          title={item.status_milik === 'M' ? 'Milik Sendiri' : 'Sewa'}
                          className={`px-3 py-1 rounded-lg text-[0.65rem] uppercase tracking-widest border ${item.status_milik === 'M' ? 'bg-blue-50 border-blue-200 text-blue-600' : 'bg-slate-100 border-slate-200 text-slate-500'}`}
                        >
                      ...
                      <td className="px-4 py-4 border-r border-slate-200 text-center">
                        <span 
                          ...
                          className={`px-3 py-1 rounded-lg text-[0.65rem] uppercase tracking-widest border ${item.status_lisensi === 'L' ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : item.status_lisensi === 'P' ? 'bg-violet-50 border-violet-200 text-violet-600' : 'bg-red-50 border-red-200 text-red-600'}`}
                        >
                          {item.status_lisensi}
                        </span>
                      </td>
                      <td className="px-6 py-4 border-r border-slate-200 text-left text-[0.75rem] text-slate-600 whitespace-normal max-w-[200px] leading-relaxed italic">{item.perangkat || '-'}</td>
                      <td className="px-6 py-4 border-r border-slate-200 text-left text-[0.75rem] text-slate-600 whitespace-normal">{item.info_tambahan || '-'}</td>
                      <td className="px-4 py-4 border-r border-slate-200 text-center">
                        {item.link_bukti ? (
                      ...
                      <td className="px-4 py-4 align-middle text-center border-r border-slate-200">
                        {showTrash ? (
                           <div className="flex items-center justify-center gap-2">
                             <button onClick={() => handleRestore(item.id_5_2)} className="p-2 bg-white border border-slate-200 text-emerald-600 hover:bg-emerald-50 hover:border-emerald-200 shadow-sm rounded-lg transition" title="Restore"><RotateCcw size={17} /></button>
                             <button onClick={() => handleHardDelete(item.id_5_2)} className="p-2 bg-white border border-slate-200 text-red-600 hover:bg-red-50 hover:border-red-200 shadow-sm rounded-lg transition" title="Hapus Permanen"><Trash2 size={17} /></button>
                           </div>
                        ) : (
                          <div className="flex justify-center gap-2">
                            ...
                            <button onClick={() => handleSoftDelete(item.id_5_2)} className="p-2 bg-white border border-slate-200 text-red-600 hover:bg-red-50 hover:border-red-200 shadow-sm rounded-lg transition-all duration-300 border border-transparent hover:border-red-100" title="Hapus"><Trash2 size={17} /></button>
                          </div>
                        )}
                      </td>
                    </tr>
