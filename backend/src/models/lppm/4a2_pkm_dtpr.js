...
const ModelPkm = {
    ...
    createTransaction: async (data, userId) => {
        ...
        try {
            ...
            // A. Insert Tabel Induk: 4A2
            const sqlInduk = `
                INSERT INTO \`4a2_pkm_dtpr\` 
                (id_roadmap, id_dosen, id_tahun, judul_pkm, jumlah_mahasiswa, jenis_hibah, sumber, durasi, jumlah_dana, link_bukti, created_by)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `;
            const [resInduk] = await connection.execute(sqlInduk, [
                ...
                data.pkm.id_tahun,
                data.pkm.judul_pkm,
                data.pkm.jumlah_mahasiswa || 0,
                data.pkm.jenis_hibah || null,
                data.pkm.sumber || null,
                data.pkm.durasi || null,
                data.pkm.jumlah_dana || null,
                data.pkm.link_bukti || null,
                userId
            ]);
            
            const id_4a2 = resInduk.insertId;

            // B. Insert Tabel Kerjasama (Jika Ada)
            if (data.kerjasama && data.kerjasama.length > 0) {
                const sqlKerjasama = `
                    INSERT INTO \`4c1_kerjasama_pkm\` 
                    (id_4a2, id_tahun, judul_kerjasama, mitra_kerja_sama, sumber, durasi, jumlah_dana, link_bukti, created_by)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                `;
                for (let k of data.kerjasama) {
                    await connection.execute(sqlKerjasama, [
                        id_4a2, k.id_tahun, k.judul_kerjasama, k.mitra_kerja_sama, 
            ...
            if (data.publikasi && data.publikasi.length > 0) {
                ...
                for (let p of data.publikasi) {
                    await connection.execute(sqlPublikasi, [
                        id_4a2, p.id_tahun, p.judul_publikasi, p.jenis_publikasi || null, p.link_bukti || null, userId
                    ]);
                }
            }
            ...
            if (data.hki && data.hki.length > 0) {
                const sqlHki = `
                    INSERT INTO \`4c3_perolehan_hki_pkm\` 
                    (id_4a2, id_tahun, judul_hki, jenis_hki, link_bukti, created_by)
                    VALUES (?, ?, ?, ?, ?, ?)
                `;
                for (let h of data.hki) {
                    await connection.execute(sqlHki, [
                        id_4a2, h.id_tahun, h.judul_hki, h.jenis_hki, h.link_bukti || null, userId
                    ]);
                }
            }
    ...
    updateTransaction: async (id, data, userId) => {
        ...
        try {
            ...
            // A. Update Tabel Induk: 4A2
            const sqlInduk = `
                UPDATE \`4a2_pkm_dtpr\`
                SET id_roadmap = ?, id_dosen = ?, id_tahun = ?, judul_pkm = ?, jumlah_mahasiswa = ?, jenis_hibah = ?, sumber = ?, durasi = ?, jumlah_dana = ?, link_bukti = ?, updated_by = ?
                WHERE id_4a2 = ?
            `;
            await connection.execute(sqlInduk, [
                data.pkm.id_roadmap, data.pkm.id_dosen, data.pkm.id_tahun, data.pkm.judul_pkm, 
                data.pkm.jumlah_mahasiswa || 0, data.pkm.jenis_hibah || null, data.pkm.sumber || null, 
                data.pkm.durasi || null, data.pkm.jumlah_dana || null, data.pkm.link_bukti || null, userId, id
            ]);
    ...
    softDelete: async (id, userId) => {
        const sql = `UPDATE \`4a2_pkm_dtpr\` SET deleted_at = CURRENT_TIMESTAMP, deleted_by = ? WHERE id_4a2 = ?`;
        const [result] = await db.execute(sql, [userId, id]);
        return result;
    },

    restore: async (id) => {
        const sql = `UPDATE \`4a2_pkm_dtpr\` SET deleted_at = NULL, deleted_by = NULL WHERE id_4a2 = ?`;
        const [result] = await db.execute(sql, [id]);
        return result;
    },

    hardDelete: async (id) => {
        const sql = `DELETE FROM \`4a2_pkm_dtpr\` WHERE id_4a2 = ?`;
        const [result] = await db.execute(sql, [id]);
        return result;
    }
};

module.exports = ModelPkm;
