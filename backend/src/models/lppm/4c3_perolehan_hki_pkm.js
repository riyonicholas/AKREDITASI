...
const ModelPkmHki = {
    ...
    softDelete: async (id, userId) => {
        const sql = `UPDATE \`4c3_perolehan_hki_pkm\` SET deleted_at = CURRENT_TIMESTAMP, deleted_by = ? WHERE id_4c3 = ?`;
        const [result] = await db.execute(sql, [userId, id]);
        return result;
    },

    restore: async (id) => {
        const sql = `UPDATE \`4c3_perolehan_hki_pkm\` SET deleted_at = NULL, deleted_by = NULL WHERE id_4c3 = ?`;
        const [result] = await db.execute(sql, [id]);
        return result;
    },

    hardDelete: async (id) => {
        const sql = `DELETE FROM \`4c3_perolehan_hki_pkm\` WHERE id_4c3 = ?`;
        const [result] = await db.execute(sql, [id]);
        return result;
    }
};

module.exports = ModelPkmHki;
